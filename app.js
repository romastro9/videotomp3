...
const staticKm = {
  ...
  featureCopy: 'កម្មវិធីបម្លែងងាយប្រើ និងមិនរក្សាទុកវីដេអូលើ Server។',
  ...
  fastCopy: 'ជ្រើសរើស កំណត់ បម្លែង ស្តាប់ និងទាញយកនៅក្នុងទីតាំងតែមួយ។',
  howKicker: 'របៀបប្រើ',
...
function cacheEnglishText() {
  $$('[data-i18n]').forEach((node) => englishText.set(node, node.textContent));
...
function applyLanguage() {
  $$('[data-i18n]').forEach((node) => {
    ...
    node.textContent = state.language === 'km' && staticKm[key] ? staticKm[key] : englishText.get(node);
  });
  $$('[data-i18n-aria]').forEach((node) => {
    const key = node.dataset.i18nAria;
    node.setAttribute('aria-label', state.language === 'km' && staticKm[key] ? staticKm[key] : node.dataset.englishAria);
  });
...
function showStage(stage) {
  [elements.uploadStage, elements.fileStage, elements.progressStage, elements.resultStage]
    .forEach((item) => item.classList.add('hidden'));
  stage.classList.remove('hidden');
}

function setProgress(percent, title = message('converting'), description = message('extracting')) {
...
function resetConverter() {
  ...
  elements.qualityOptions.forEach((button) => {
    button.classList.toggle('active', Number(button.dataset.quality) === state.quality);
  });
...
async function selectFile(file) {
  ...
  if (!isMp4) {
    showAlert(message('invalidType'));
    return;
  }
  if (file.size > MAX_FILE_SIZE) {
    showAlert(message('tooLarge'));
    return;
  }
  ...
  try {
    state.duration = await readVideoDuration(state.fileUrl);
  } catch {
    clearInput();
    showAlert(message('metadataError'));
    return;
  }
  ...
  if (file.size >= LARGE_FILE_SIZE) showAlert(message('largeWarning'));
...
function updateFfmpegProgress({ ratio }) {
  ...
  setProgress(Math.min(96, Math.max(5, ratio * 96)));
}

async function loadFfmpeg() {
  if (state.ffmpeg && state.ffmpegLoaded) return state.ffmpeg;
  if (!window.FFmpeg || typeof window.FFmpeg.createFFmpeg !== 'function') {
    throw new Error('engine-script');
  }

  const instance = window.FFmpeg.createFFmpeg({
    corePath: CORE_PATH,
    log: false,
    progress: updateFfmpegProgress
  });
  if (typeof instance.setProgress === 'function') instance.setProgress(updateFfmpegProgress);

  state.ffmpeg = instance;
  await instance.load();

  if (state.cancelRequested || state.ffmpeg !== instance) {
    try { instance.exit(); } catch { /* Worker may already be closed. */ }
...
function removeVirtualFile(ffmpeg, path) {
  try {
    ffmpeg.FS('unlink', path);
  } catch {
    // Missing virtual files are expected after cancellation or failed conversion.
  }
}
...
async function convertFile() {
  ...
  const virtualInput = `input-${stamp}.mp4`;
  const virtualOutput = `output-${stamp}.mp3`;

  state.converting = true;
  state.cancelRequested = false;
  clearOutput();
  showStage(elements.progressStage);
  setProgress(1, message('loadingEngine'), message('loadingEngineCopy'));
  ...
  try {
    ...
    ffmpeg.FS('writeFile', virtualInput, await window.FFmpeg.fetchFile(state.file));
    ...
    setProgress(5, message('converting'), message('extracting'));
    await ffmpeg.run(...args);
    if (state.cancelRequested) throw new Error('cancelled');

    setProgress(98, message('preparing'), message('cleaning'));
    ...
    removeVirtualFile(ffmpeg, virtualInput);
    removeVirtualFile(ffmpeg, virtualOutput);
    setProgress(100);
    ...
    const engineFailure = String(error.message).includes('engine') || String(error.message).includes('load');
    showAlert(cancelled ? message('cancelled') : (engineFailure ? message('engineError') : message('conversionError')));
  } finally {
...
function cancelConversion() {
  ...
  const instance = state.ffmpeg;
  state.ffmpeg = null;
  state.ffmpegLoaded = false;
  try {
    if (instance && typeof instance.exit === 'function') instance.exit();
  } catch {
...
function bindEvents() {
  ...
  elements.qualityOptions.forEach((button) => {
    button.addEventListener('click', () => {
      ...
      elements.qualityOptions.forEach((option) => option.classList.toggle('active', option === button));
    });
  });
  ...
  elements.themeButton.addEventListener('click', () => {
    ...
    elements.themeButton.setAttribute('aria-label', dark ? 'Use dark mode' : 'Use light mode');
  });
  ...
  window.addEventListener('beforeunload', () => {
    ...
    clearInput();
    try {
      if (state.ffmpeg && typeof state.ffmpeg.exit === 'function') state.ffmpeg.exit();
    } catch {
